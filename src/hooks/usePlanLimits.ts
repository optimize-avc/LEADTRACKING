...
export function usePlanLimits(): UsePlanLimitsResult {
  ...
  const remainingLeadsThisMonth = getRemainingUsage(currentUsage.leadsThisMonth, limits.leadsPerMonth);
  const remainingTeamMembers = getRemainingUsage(currentUsage.teamMembers, limits.teamMembers);
  const remainingEmailsThisMonth = getRemainingUsage(currentUsage.emailsThisMonth, limits.emailsPerMonth);
