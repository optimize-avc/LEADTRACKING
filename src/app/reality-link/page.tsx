...
export default function RealityLinkPage() {
    ...
    useEffect(() => {
        ...
        const matched = companyResources.filter((res) => {
            ...
            if (
                res.title.toLowerCase().includes('competitor') &&
                lastSentence.includes('competitor')
            )
                return true;
            if (res.title.toLowerCase().includes('security') && lastSentence.includes('security'))
                return true;
            if (
                res.title.toLowerCase().includes('pricing') &&
                (lastSentence.includes('price') || lastSentence.includes('cost'))
            )
                return true;
        ...
        if (matched.length > 0) {
            setActiveCards((prev) => {
                ...
                if (newCards.length > 0) {
                    // Flash effect
                    toast(
                        `${newCards.length} Battle Card${newCards.length > 1 ? 's' : ''} Detected!`
                    );
                }
            ...
            matched.forEach((res) => {
                if (res.title.toLowerCase().includes('competitor')) {
                    newTactics.push(
                        "Strategy: Use 'The Differentiator' script to highlight our unique AI moat."
                    );
                }
                if (res.category.toLowerCase().includes('pricing')) {
                    newTactics.push(
                        "Tactic: Shift focus to 'Cost of Inaction' rather than Implementation Fee."
                    );
                }
                if (res.title.toLowerCase().includes('security')) {
                    newTactics.push(
                        'Advice: Mention our dual-layer encryption and ISO compliance immediately.'
                    );
                }
            });
    ...
    const startSimulation = (type: 'competitor' | 'security' | 'pricing') => {
        ...
        if (type === 'competitor') {
            script =
                "Yeah so we've been looking at Competitor X recently. They seem to have good features. Also I heard their pricing is lower.";
        } else if (type === 'security') {
            script =
                'My main concern right now is compliance. We have strict ISO 27001 requirements. How do you handle data security?';
        } else if (type === 'pricing') {
            script =
                "Basically we don't have budget for this. Your implementation costs are just too high compared to what we're paying now.";
        }
            ...
            <header className="relative z-10 flex justify-between items-center mb-8 pb-4 border-b border-white/5">
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <div
                            className={`w-3 h-3 rounded-full ${isListening ? 'bg-red-500 animate-pulse' : 'bg-slate-600'}`}
                        ></div>
                        {isListening && (
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-white tracking-widest uppercase flex items-center gap-2">
                            Reality Link{' '}
                            <span className="text-xs bg-blue-900/50 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30">
                                V.1.0
                            </span>
                        </h1>
                        <p className="text-xs text-slate-500 font-mono">
                            LIVE AUDIO INTELLIGENCE FEED
                        </p>
                    </div>
                </div>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10 min-h-0">
                ...
                <div className="lg:col-span-6 flex flex-col gap-6">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2">
                        <button
                            onClick={() => startSimulation('competitor')}
                            className="px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 hover:border-blue-500 hover:text-blue-400 text-xs font-mono uppercase transition-colors whitespace-nowrap"
                        >
                            Simulate: Competitor Mention
                        </button>
                        <button
                            onClick={() => startSimulation('security')}
                            className="px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 hover:border-orange-500 hover:text-orange-400 text-xs font-mono uppercase transition-colors whitespace-nowrap"
                        >
                            Simulate: Security Audit
                        </button>
                        <button
                            onClick={() => startSimulation('pricing')}
                            className="px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 hover:border-green-500 hover:text-green-400 text-xs font-mono uppercase transition-colors whitespace-nowrap"
                        >
                        ...
                        <div className="font-mono text-lg text-slate-300 leading-relaxed whitespace-pre-wrap">
                            {transcript || (
                                <span className="text-slate-700 animate-pulse">
                                    Waiting for audio speech...
                                </span>
                            )}
                            {isListening && (
                                <span className="inline-block w-2.5 h-5 bg-blue-500 ml-1 animate-blink align-middle"></span>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-3 flex flex-col gap-6 h-full min-h-0">
                    ...
                    <div className="flex flex-col gap-4">
                        {aiTactics.length === 0 ? (
                            ...
                            aiTactics.map((tactic, i) => (
                                <div
                                    key={i}
                                    className="p-4 rounded-xl border border-blue-500/20 bg-blue-500/5 animate-in slide-in-from-top-2"
                                >
                    ...
                    <h2 className="text-sm font-bold text-slate-400 opacity-50 uppercase tracking-widest mt-4">
                        Active Lead Context
                    </h2>
                        ...
                        {activeCards.length === 0 ? (
                            <div className="h-64 border-2 border-dashed border-slate-800 rounded-xl flex flex-col items-center justify-center text-center p-6 opacity-50">
                                ...
                                <p className="text-xs text-slate-600 mt-2">
                                    AI will surface relevant enablement materials here
                                    automatically.
                                </p>
                            </div>
                        ) : (
