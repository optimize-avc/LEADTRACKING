  ...
  try {
    ...
    if (!companyId || !inviteId) {
      return NextResponse.json(
        { error: 'Missing companyId or inviteId' },
        { status: 400 }
      );
    }
    ...
    const inviteRef = db.collection('companies').doc(companyId).collection('invites').doc(inviteId);
    ...
    const teamMemberRef = db.collection('companies').doc(companyId).collection('team').doc(user.uid);
    ...
    console.log(`[API] Invite accepted: User ${user.uid} joined company ${companyId} as ${invite?.role}`);
