...
function NewsArtifact({ data }: { data: any }) {
    return (
        <div className="bg-white text-black p-8 rounded-sm shadow-2xl skew-y-1 transform transition-all hover:skew-y-0">
            ...
            <div className="flex gap-6">
                <div className="w-1/3 bg-gray-200 h-48 rounded-sm flex items-center justify-center overflow-hidden grayscale">
                    <img src={data.imageUrl} className="w-full h-full object-cover mix-blend-multiply" alt="News" />
                </div>
...
function EmailArtifact({ data }: { data: any }) {
    return (
            ...
            <div className="p-8">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="font-bold text-lg">{data.from}</div>
                            <div className="text-gray-500 text-sm">to {data.to}</div>
                        </div>
                    </div>
...
function SlackArtifact({ data }: { data: any }) {
    return (
...
function StockArtifact({ data }: { data: any }) {
    return (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="h-32 w-full flex items-end gap-1 mb-8">
                {[...Array(20)].map((_, i) => (
                    <div key={i} className="flex-1 bg-red-500/20 hover:bg-red-500/50 transition-colors" style={{ height: `${Math.random() * 80 + 20}%` }}></div>
                ))}
            </div>
