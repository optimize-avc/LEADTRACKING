...
export default function BillingPage() {
  ...
  const leadsPercentage = limits.leads === -1 
    ? 0 
    : Math.min(100, (usage.leads / limits.leads) * 100);
  const teamPercentage = limits.teamMembers === -1
    ? 0
    : Math.min(100, (usage.teamMembers / limits.teamMembers) * 100);
  const emailsPercentage = limits.emailsPerMonth === -1
    ? 0
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    {planInfo.price === '$0' ? 'Free forever' : `${planInfo.price} billed monthly`}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: limits.leads === -1 ? '5%' : `${leadsPercentage}%` }}
                  />
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm text-slate-400">
                    ...
                    {remainingTeamMembers > 0 && remainingTeamMembers !== -1 && (
                      <span className="text-slate-500 ml-2">
                ...
                <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: limits.teamMembers === -1 ? '5%' : `${teamPercentage}%` }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-300">Emails This Month</span>
                  </div>
                  <span className="text-sm text-slate-400">
                    {usage.emailsThisMonth.toLocaleString()} / {formatLimit(limits.emailsPerMonth)}
                    {remainingEmailsThisMonth > 0 && remainingEmailsThisMonth !== -1 && (
                      <span className="text-slate-500 ml-2">
                        ({remainingEmailsThisMonth.toLocaleString()} remaining)
                      </span>
                    )}
                  </span>
                </div>
                <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: limits.emailsPerMonth === -1 ? '5%' : `${emailsPercentage}%` }}
                  />
                </div>
              </div>
            </div>
        ...
        >
          <GlassCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FeatureRow 
                label="AI Deal Wargaming" 
                enabled={limits.aiFeatures} 
              />
              <FeatureRow 
                label="Custom Lead Fields" 
                enabled={limits.customFields} 
              />
              <FeatureRow 
          ...
          <GlassCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-1">See all features by tier</p>
              </a>
              ...
              {tier !== 'free' && (
                ...
                >
                  <h4 className="text-sm font-medium text-white">View Invoices</h4>
                  <p className="text-xs text-slate-500 mt-1">Access in Stripe Portal</p>
                </button>
              )}
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-1">Get help with payments</p>
              </a>
            </div>
          </GlassCard>
        </motion.div>
