...
const ACTION_DECK: ActionCard[] = [
    {
        id: 'whitepaper',
        label: 'Send Whitepaper',
        icon: <FileText size={16} />,
        risk: 'Low',
        description: 'Safe, standard move. Good for analytical types.',
        cost: 1,
    },
    {
        id: 'demo',
        ...
        icon: <Play size={16} />,
        risk: 'Low',
        description: 'Show the product. effective if interest is real.',
        cost: 2,
    },
    {
        id: 'social',
        ...
        risk: 'Medium',
        description: 'Dig for hidden agendas via backchannels.',
        cost: 2,
    },
    {
        id: 'ciso',
        label: 'CISO-to-CISO Call',
        icon: <Lock size={16} />,
        risk: 'Medium',
        description: 'High impact for security blockers.',
        cost: 3,
    },
    {
        id: 'executive',
        ...
        risk: 'High',
        description: 'Go over their head. Huge risk, huge reward.',
        cost: 4,
    },
];
...
export function WarRoomRunner({ onClose, initialContext }: WarRoomRunnerProps) {
    ...
    const startGame = async () => {
        ...
        try {
            ...
            const scenario = await GeminiService.generateDealScenario(
                level,
                leadContext,
                contextContent,
                token
            ...
            if (scenario) {
                setGameState((prev) => ({
                    ...
                    stakeholders: (scenario.stakeholders || []).map((s: AIStakeholder) => ({
                        ...s,
            ...
            if (result) {
                ...
                setGameState((prev) => {
                    const newHealth = Math.max(
                        0,
                        Math.min(100, prev.dealHealth + (result.dealHealthDelta || 0))
                    );
                    const newStatus =
                        newHealth <= 0
                            ? 'lost'
                    ...
                    const updatedStakeholders = prev.stakeholders.map((s) =>
                        s.id === selectedStakeholderId
                            ? {
                                  ...
                                  sentiment: Math.max(
                                      0,
                                      Math.min(100, s.sentiment + (result.sentimentDelta || 0))
                                  ),
                              }
                            : s
                    );
                ...
                <div className="max-w-2xl w-full bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-slate-400">
                            Configure your simulation environment. Run &ldquo;Crisis Ops&rdquo; on
                            live deals or standard training.
                        </p>
                    </div>
                            ...
                            {loadingResources ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                    {resources.map((res) => (
                                        ...
                                        >
                                            <FileText
                                                className={`w-5 h-5 ${selectedResourceId === res.id ? 'text-blue-400' : 'text-slate-500'}`}
                                            />
                            ...
                            {leads.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                                    {leads.map((lead) => (
                                        ...
                                        >
                                            <ShieldAlert
                                                className={`w-5 h-5 ${selectedLeadId === lead.id ? 'text-red-500' : 'text-slate-600'}`}
                                            />
                                            <div className="flex-1 min-w-0">
                                                <div className="font-medium truncate text-sm">
                                                    {lead.companyName}
                                                ...
                                                <div className="text-xs opacity-60 truncate">
                                                    ${lead.value?.toLocaleString()} • {lead.status}
                                                </div>
                                            </div>
                                            {selectedLeadId === lead.id && (
            ...
            <div className="fixed inset-0 left-64 z-50 flex items-center justify-center bg-slate-950">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-orange-400 font-mono text-sm">
                        Generating Procedural Scenario via Gemini 2.5 Flash...
                    </p>
                </div>
            </div>
        );
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-sm relative">
                        {gameState.logs.map((log) => (
                            ...
                            >
                                ...
                                <div
                                    className={`p-3 rounded border border-l-2 ${
                                        log.type === 'system'
                                            ? 'border-slate-800 border-l-slate-500 bg-slate-900/50'
                                            : (log.impact || 0) > 0
                                              ? 'border-green-900/30 border-l-green-500 bg-green-950/10'
                ...
                <div className="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-lg flex items-center justify-center animate-in fade-in duration-500">
                    <div className="max-w-md w-full glass-card p-8 text-center border-orange-500/30">
                        ...
                        <h2 className="text-4xl font-bold text-white mb-2">
                            {gameState.status === 'won' ? 'DEAL CLOSED' : 'DEAL LOST'}
                        </h2>
