...
import { rateLimit, errorResponse, getUserIdFromToken, handleValidationError } from '@/lib/api-middleware';
...
async function getUserPlanAndUsage(userId: string) {
  ...
  // Get company usage
  const usageDoc = await db.collection('companies').doc(companyId).collection('meta').doc('usage').get();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const usageRef = db.collection('companies').doc(effectiveCompanyId).collection('meta').doc('usage');
